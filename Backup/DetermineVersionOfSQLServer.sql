...
SELECT	@ProductName = 
		...
		@TheLastVersion = 
		CASE SUBSTRING(@ProductVersion,1,4)
			WHEN '12.0' THEN 'SQL Server 2014 SP1 CU4' 
			WHEN '11.0' THEN 'SQL Server 2012 SP3 CU1' 
...
DECLARE @Temp1 NVARCHAR(100) = 'You have already installed the latest service pa1ck.' 
...
SELECT  @SPInfo = 
		...
		CASE @ProductName
				WHEN 'SQL Server 2014' THEN 
											CASE @ProductVersion
												WHEN '12.0.4436.0' THEN 'You have already installed the latest cumulative update.'
												ELSE 'Install the latest Cumulative Update (CU) of SP1:  CU4, <https://support.microsoft.com/en-us/kb/3106660>'
											END
				--WHEN 'SQL Server 2012' THEN
				--							CASE @ProductVersion
				--								WHEN '' THEN 'You have already installed the latest cumulative update.'
				--								ELSE 'Install the latest Cumulative Update (CU) of SP2:  CU8, <https://support.microsoft.com/en-us/kb/3082561>'
				--							END
...
BEGIN
	...
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '11.0.3482.00')
	BEGIN
		SET @CumulativeUpdate = 'CU13'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3002044';
	END
	ELSE IF (@ProductLevel = 'SP2') AND (@ProductVersion < '11.0.5644.0')
	BEGIN
		SET @CumulativeUpdate = 'CU10'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3120313';
	END
	ELSE IF (@ProductLevel = 'SP3') AND (@ProductVersion < '11.0.6518.0')
	BEGIN
		SET @CumulativeUpdate = 'CU1'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3123299';
	END
	SELECT  @OtherProduct = 
			CASE 
				WHEN @ProductLevel = 'RTM' THEN ' If you want to upgrade to higher edition, please intall service pack.'
...
BEGIN
	...
	--2014
	IF (@ProductLevel = 'RTM') AND (@ProductVersion < '12.0.2560.0')
	BEGIN
		SET @CumulativeUpdate = 'CU11'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3106659';
	END
	ELSE IF (@ProductLevel = 'SP1') AND (@ProductVersion < '12.0.4436.0')
	BEGIN
		SET @CumulativeUpdate = 'CU4'; SET @CumulativeUpdateKB = 'https://support.microsoft.com/en-us/kb/3106660';
	END
